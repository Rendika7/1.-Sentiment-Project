...
indonesian_roberta_base_emotion_classifier = pipeline(
...
with st.container():
    ...
    with text_input_col:
        text_input = st.text_area("Masukkan teks untuk emotion sentiment:")

    # Fungsi untuk melakukan prediksi sentimen
    def predict_sentiment(text):
        scores = indonesian_roberta_base_emotion_classifier(text)
...
def main():
    ...
    st.write("Emotion analysis for text in a dataframe")
    ...
    if st.button("Try the Features"):
        ...
        for i, text in sampled_df['JAWABAN'].items():
            results = indonesian_roberta_base_emotion_classifier(text)
            max_score_index = np.argmax([entry['score'] for entry in results[0]])
            
            predicted_sentiment = results[0][max_score_index]['label']
            sentiment_score = results[0][max_score_index]['score']
            
            predicted_sentiments.append(predicted_sentiment)
            sentiment_prediction_score.append(sentiment_score)

        sampled_df['Predicted_Sentiment'] = predicted_sentiments
        sampled_df['Score_Conffidence'] = sentiment_prediction_score
        # Tampilkan DataFrame setelah prediksi sentimen
        st.table(sampled_df[['JAWABAN', 'Predicted_Sentiment', 'Score_Conffidence']])        
